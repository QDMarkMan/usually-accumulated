 ...
 * 方法返回会一个由替换之替换一些或者所有匹配模式后得字符串
 * 模式可以是一个字符串或者一个正正则表达式，替换之可以是一个字符串或者一个函数
 * 注意：返回一个新字符串而且原字符串不会改变！
 * regexp (pattern)
 ...
 */
 ...
 /**
  * ⭐⭐⭐⭐⭐
  * slice(beginSlice,endSlice) 参数负数是倒数
  * 提取字符串的一部分并返回一个新的字符串 
  * beginSlice:开始位置
  * endSlice：结束位置 
  * 注意 ：不会修改原来的字符串 返回一个新的字符串  在截取的时候包括begin但是不包括end  同样的要头不要尾
  */
  ...
  * separator：分割符
  * limit：限制分割的数量
  * 注意：返回的是一个新数组，不会修改原字符串 如果空字符串("")被用作分隔符，则字符串会在每个字符之间分割。
 ...
 * 注意：修改的是当前的字符串，和slice不同的是slice返回一个新字符串 和substr不同的是sustr第二个参数是长度 这个是索引
